...
export class ApiVersionPipe implements PipeTransform {
  ...
  transform(value: any, metadata: ArgumentMetadata): any {
    ...
    if (metadata.type === 'param' && metadata.data === 'version') {
      return this.validateAndTransformVersion(value);
    }
   ...
   */
  private validateAndTransformVersion(version: string): string {
    if (!version) {
    ...
    this.logger.debug(
      `Transformed version parameter: ${version} -> ${versionInfo.version}`,
    );
